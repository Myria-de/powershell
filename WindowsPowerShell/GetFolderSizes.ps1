...
.NOTES
	...
	Changelog:
					Changed Sort Funktion ###te 03-2019
...
Param (
	...
	#Windows: [string[]]$Paths = "$env:USERPROFILE\Downloads",
        [string[]]$Paths = (Join-Path ([Environment]::GetFolderPath('UserProfile')) 'Downloads'),
	#Windows: [string]$ReportPath = "$env:USERPROFILE",
        [string]$ReportPath = [Environment]::GetFolderPath('UserProfile'),
...
Begin {
    Function AddObject {
    	...
    	$Object = New-Object PSObject -Property @{
    		...
    		Size = $Size
    		#Nur Windows: Owner = (Get-Acl $FileObject.FullName).Owner
                Owner = ''
            RawSize = $RawSize
    	}
        Return $Object
...
"@
    ...
    #$HTML = $Report | Sort-Object -Property Size | Select-Object -Property 'Folder Name',@{Name='Size(MB)';Expression={"{0,10:N2}" -f ($_.Size / 1MB)}} | ConvertTo-Html -PreCont $Pre -PostContent $Post -Head $Header | Set-AlternatingRows -CSSEvenClass even -CSSOddClass odd | Out-File $ReportPath\FolderSizes.html
	$HTML = $Report | Select 'Folder Name',Owner,'Created On','Last Updated',Size,RawSize | Sort-Object -Property RawSize -Descending:$Descending | ConvertTo-Html -PreCont $Pre -PostContent $Post -Head $Header | Set-AlternatingRows -CSSEvenClass even -CSSOddClass odd | Out-File $ReportPath\FolderSizes.html
